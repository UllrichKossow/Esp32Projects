...
void light_sleep_enable(void) {
    ...
    const esp_pm_config_esp32_t pm_config = {.max_freq_mhz = cur_freq_mhz, .min_freq_mhz = cur_freq_mhz, .light_sleep_enable = true};
...
void loop() {
    ...
    esp_timer_init();
    esp_timer_create_args_t create_args_slow = {.callback = timer_cb_slow, .arg = &s, .dispatch_method = ESP_TIMER_TASK, .name = "TimerSlow"};
    ...
    esp_timer_create_args_t create_args_fast = {.callback = timer_cb_fast, .arg = NULL, .dispatch_method = ESP_TIMER_TASK, .name = "TimerFast"};
