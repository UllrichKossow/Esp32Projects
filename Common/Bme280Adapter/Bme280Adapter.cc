...
void Bme280Adapter::init() {
    ...
    esp_err_t probe = i2c_master_probe(m_busHandle, dev_cfg.device_address, 1000);
    m_chip_exists = probe == ESP_OK;
    ...
    ESP_LOGI(TAG, "maxdelay=%lu", maxdelay);
}

bool Bme280Adapter::read(data_t &data) {
    if (!m_chip_exists) {
        return false;
    }
    bme280_settings settings;
    ...
    ESP_LOGD(TAG, "bme280_get_sensor_data() returned %i", rslt);
    ESP_LOGD(TAG, "temperature=%li", comp_data.temperature);
    ESP_LOGD(TAG, "humidity=%li", comp_data.humidity);
    ESP_LOGD(TAG, "pressure=%li", comp_data.pressure);
